...
│   │   │   └── molecules/
│   │   ├── hooks/
│   │   ├── stores/                 # Segmented Zustand UI stores
│   │   │   └── auth-menu.store.ts
...
The `/domains` directory organizes business logic following the Screaming Architecture philosophy. Each domain must represent a narrowly bounded business capability; generic domains are forbidden. Each domain encapsulates its own components, hooks, services, validations, specific actions, and text messages. Its `/stores/` folder contains segmented Zustand stores, each responsible for one UI capability, while `messages.ts` contains domain-specific UI text.
